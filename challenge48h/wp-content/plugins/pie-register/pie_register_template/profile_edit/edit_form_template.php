...
class Edit_form_template extends Edit_form
{
	...
	function addEmail(){
		...
		$data  = '<div class="fieldset">'.$this->addLabel();
		$data .= '<input id="'.$this->id.'" name="e_mail" data-field_id="piereg_field_'.$this->no.'" class="'.$this->addClass().'" '.$this->addValidation().' placeholder="'.$this->field['placeholder'].'" type="text" value="'.$this->user->data->user_email.'" autocomplete="off" />';
		$data .= $this->addDesc();
		$data .= '</div>';
		return $data;
	}
	...
	function addTime(){
		...
		if($this->field['time_type']=="12")
		{
			...
			$data .= '<div id="time_format_field_'.$this->id.'" class="time_fields">
				<select '.$this->read_only.' name="'.$this->name.'[time_format]" class=">
